...
function minimapDrawRotatedRect(item){
  ...
  let fill='#303a45';
  if(item.kind==='cover')fill=item.variant===2?'#725b42':'#526777';
  else if(item.kind==='crate')fill='#6c533b';
  else if(item.kind==='terminal')fill='#25758a';
  else if(item.kind==='tree')fill='#294b37';
  else if(item.impactMaterial==='metal')fill='#46545f';
  ctx.fillStyle=fill;ctx.globalAlpha=item.kind==='tree'?.78:.94;
...
function minimapDrawStatic(){
  ...
  ctx.strokeStyle='rgba(120,170,205,.08)';ctx.lineWidth=1;
...
function minimapDrawZone(zone){
  ...
  const radius=Math.max(8,zone.r*p.s),owner=frontlineZoneOwners[zone.id],active=frontlineObjective.zoneId===zone.id;
  ...
  ctx.lineWidth=active?2.6:1.3;ctx.strokeStyle=stroke;ctx.stroke();
  if(active){ctx.beginPath();ctx.arc(p.x,p.y,radius+4,0,Math.PI*2);ctx.strokeStyle='rgba(255,225,102,.36)';ctx.lineWidth=1.5;ctx.stroke();}
...
function minimapDrawTriangle(x,z,heading,color,size=5){
  ...
  ctx.fillStyle=color;ctx.shadowColor=color;ctx.shadowBlur=size*.9;ctx.fill();ctx.shadowBlur=0;ctx.restore();
}
function renderTacticalMinimap(){
  ...
  for(const pk of pickups){
    ...
    minimapCtx.fillStyle='rgba(255,200,70,.72)';minimapCtx.fillRect(p.x-1.2,p.y-1.2,2.4,2.4);
  }
  for(const bot of enemies){
    if(!bot.alive||bot.team!=='ally')continue;
    const pos=bot.group.position;minimapDrawTriangle(pos.x,pos.z,bot.group.rotation.y,'#4ab8ff',3.3);
  }
  const playerHeading=Math.atan2(-Math.sin(yaw),-Math.cos(yaw));
  minimapDrawTriangle(camera.position.x,camera.position.z,playerHeading,'#c8ffff',5.3);
